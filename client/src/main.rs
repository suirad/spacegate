...
#[command(
    about = "Spacegate enables UDP routing of a spacetimedb client either to Maincloud(default) or to a standalone STDB server when used alongside the spacegate-proxy"
)]
struct Args {
    #[arg(
        short,
        long,
        help = "The forward proxy nodeID/PublicKey to connect to (forward proxy prints it on start)"
    )]
    node: Option<String>,
...
pub async fn rewrite_host_header(
    recv: &mut ReadHalf<'_>,
